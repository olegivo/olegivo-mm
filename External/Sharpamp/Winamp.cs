...
namespace Daniel15.Sharpamp
{
	...
	public class Winamp
	{
		...
		/// </summary>
		public const string WINAMP_TITLE = "Winamp v1.x";
		...
		protected enum IPCCommand
		{
            ...
            /// <summary>
            /// Will set the current playlist item
            /// </summary>
            ChangeCurrentFile = 245,
		}
		...
		private void Init()
		{
			...
			/*#if DEBUG
				string debugDir = Environment.CurrentDirectory;
	    ...
	    /// <returns>Return value of command</returns>
	    protected IntPtr SendIPCCommand(IPCCommand command, string param)
		{
			//return SendMessage(_WinampWindow, WM_WA_IPC, (Int32) command, 0);
            IntPtr text = Marshal.StringToCoTaskMemUni(param);
			return Win32.SendMessage(_WinampWindow, WM_WA_IPC, text, (Int32)command);
		}
	    ...
	    {
	        SendIPCCommand(IPCCommand.ChangeCurrentFile, playlistFilename);
	    }
	}
