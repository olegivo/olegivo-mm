...
>
  ...
  <xsl:template match="wix:Wix">
    ...
    <xsl:variable name="binaries" select="wix:Fragment/wix:DirectoryRef/wix:Directory/wix:Component[(substring(wix:File/@Source, string-length(wix:File/@Source) - 2) = 'dll' or substring(wix:File/@Source, string-length(wix:File/@Source) - 2) = 'exe') 
                  and not(contains(wix:File/@Source, 'vshost')) and not(contains(wix:File/@Source, 'Telerik.Windows.Controls.resources.dll')) ]"/>
  ...
  <xsl:template match="Fragment/DirectoryRef/Component">
  </xsl:template>
  ...
  <xsl:template match="wix:Component">
    <Component Id="{@Id}" Guid="{@Guid}">
      <File Id="{wix:File/@Id}" Source="{concat('..\Oleg_ivo.Melomanager\bin\Debug', substring(wix:File/@Source, 10))}" />
    </Component>
  </xsl:template>
